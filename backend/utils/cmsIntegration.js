...
class CMSIntegration {
  ...
  async publishToWordPress(credentials, content) {
    console.log('🔍 WordPress publishToWordPress called with:', {
      siteUrl: credentials.authDetails?.siteUrl,
      username: credentials.authDetails?.username,
      hasApplicationPassword: !!credentials.authDetails?.applicationPassword,
    ...
    if (!siteUrl || !username || !applicationPassword) {
      throw new Error('Missing WordPress credentials: siteUrl, username, and applicationPassword are required');
    }
    ...
    const plainTextContent = convertedContent.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
    const excerpt = plainTextContent.substring(0, 150);
    
    // Handle keywords - convert to tags or categories
    let keywordArray = [];
    if (content.keywords) {
      if (Array.isArray(content.keywords)) {
        keywordArray = content.keywords;
      } else if (typeof content.keywords === 'string') {
        keywordArray = content.keywords.split(',').map(k => k.trim()).filter(k => k.length > 0);
      }
    }

    // Create enhanced content with metadata
    const enhancedContent = `${convertedContent}

<!-- SEO Metadata -->
<!-- Target Audience: ${content.targetAudience || 'General Audience'} -->
<!-- Keywords: ${keywordArray.join(', ')} -->
<!-- Generated by Snowball AI Content Creator -->`;
    ...
    console.log('📝 Preparing WordPress post data:', {
      ...
      status: postStatus,
      keywordsCount: keywordArray.length,
      targetAudience: content.targetAudience
    });

    try {
      // First, try to create/get tags from keywords
      let tagIds = [];
      if (keywordArray.length > 0) {
        try {
          for (const keyword of keywordArray.slice(0, 10)) { // Limit to 10 tags
            try {
              // Try to get existing tag first
              const existingTagsResponse = await axios.get(`${siteUrl}/wp-json/wp/v2/tags?search=${encodeURIComponent(keyword)}`, {
                headers: { 'Authorization': `Basic ${auth}`, 'Content-Type': 'application/json' }
              });
              
              let tagId;
              if (existingTagsResponse.data.length > 0) {
                tagId = existingTagsResponse.data[0].id;
                console.log(`Found existing tag: ${keyword} (ID: ${tagId})`);
              } else {
                // Create new tag
                const newTagResponse = await axios.post(`${siteUrl}/wp-json/wp/v2/tags`, {
                  name: keyword,
                ...
                console.log(`Created new tag: ${keyword} (ID: ${tagId})`);
              }
              ...
              console.warn(`Failed to handle tag "${keyword}":`, tagError.message);
              // Continue with other tags even if one fails
            }
          }
        } catch (tagProcessingError) {
          console.warn('Error processing tags, continuing without tags:', tagProcessingError.message);
        }
      }
      ...
      const postData = {
        ...
        tags: tagIds,
        meta: {
          target_audience: content.targetAudience || 'General Audience',
          seo_keywords: keywordArray.join(', '),
          generated_by: 'Snowball AI'
        }
      };

      // Handle categories if provided
      if (content.categories && Array.isArray(content.categories) && content.categories.length > 0) {
        postData.categories = content.categories;
      }

      console.log('🚀 Sending WordPress API request:', {
      ...
      return {
        ...
        url: response.data.link,
        status: response.data.status,
        title: response.data.title?.rendered || cleanTitle
      };
      ...
      if (error.response?.data?.message) {
        ...
        errorMessage = 'Permission denied. Please ensure your WordPress user has permission to publish posts.';
